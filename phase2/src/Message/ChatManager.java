...
public class ChatManager implements Serializable {
    ...
    private ArrayList<Chat> archivedChatsList; // List for storing collection of archived Chat objects

    public ChatManager() {
        chatsList = new ArrayList<>();
        aChatsList = new ArrayList<>();
        archivedChatsList = new ArrayList<>();
    }
    ...
    public String findChat(String currentId, ArrayList<String> guestsId) {
        ...
        if (!(guestsId.contains(currentId))) {
    ...
    /**
     * Return how many people are in that Chat
     *
     * @param chatId Id of the Chat
     * @return integer corresponding to number of Person participating to that Chat
     */
    public int getChatSize(String chatId) {
        return Objects.requireNonNull(this.getChat(chatId)).getPersonIds().size();
    }

    /**
     * Make ID of a Chat null
     *
     * @param chatId ID of the chat
     * @return true iff Chat ID nullified
     */
    ...
    /**
     * Move a chat from chatList to archivedChatsList
     *
     * @param chatId Id of the Chat
     * @return true iff chat moved correctly
     */
    public boolean archiveChat(String chatId) {
        Chat chat = getUnknownTypeChat(chatId);
        if (chatsList.contains(chat)){
            chatsList.remove(chat);
            archivedChatsList.add(chat);
            return true;
        }else if (aChatsList.contains(chat)){
            aChatsList.remove(chat);
            archivedChatsList.add(chat);
            return true;
        } return false;}

    public boolean dearchiveChat(String chatId){
        Chat chat = getUnknownTypeChat(chatId);
        Class<?> chatClass = getChatClass(chatId);
        if ((chatClass == Chat.class)&&(chatsList.contains(chat))){
            archivedChatsList.remove(chat);
            chatsList.add(chat);
            return true;
        }else if ((chatClass == AnnouncementChat.class)&&(aChatsList.contains(chat))){
            archivedChatsList.remove(chat);
            aChatsList.add((AnnouncementChat)chat);
            return true;
        } return false;}
}
